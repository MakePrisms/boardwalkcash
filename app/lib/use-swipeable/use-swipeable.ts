...
const defaultProps: ConfigurationOptions = {
  ...
  trackMouse: false,
  trackTouch: true,
  swipeDuration: Number.POSITIVE_INFINITY,
      ...
      const delta =
        typeof props.delta === 'number'
          ...
          : props.delta[dir.toLowerCase() as Lowercase<SwipeDirections>] ||
            defaultProps.delta;
      if (
        absX < (delta as number) &&
        absY < (delta as number) &&
        !state.swiping
      )
        return state;
...
export function useSwipeable(options: SwipeableProps): SwipeableHandlers {
  ...
  for (defaultKey in defaultProps) {
    if (transientProps.current[defaultKey] === void 0) {
      (transientProps.current[
        defaultKey
      ] as ConfigurationOptions[typeof defaultKey]) = defaultProps[defaultKey];
    }
  }
  ...
  const [handlers, attachTouch] = React.useMemo(
    () =>
      getHandlers(
        (stateSetter) => {
          const nextState = stateSetter(
            transientState.current,
            transientProps.current,
          );
          transientState.current = nextState;
          return nextState;
        },
        { trackMouse },
      ),
