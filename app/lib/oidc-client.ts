 ...
 */
export async function prepareOidcFlow({
  openIdDiscoveryUrl,
  clientId,
  redirectUri,
  ...
  redirectUri: string;
  scope?: string;
  customStateData?: Record<string, unknown>;
}): Promise<URL> {
  ...
  sessionStorage.setItem('oidc_security_state', securityState);
...
export async function completeOidcFlow(callbackUrl: string): Promise<{
  ...
  const storedSecurityState = sessionStorage.getItem('oidc_security_state');
  ...
  try {
    ...
    const encodedCallbackState = urlParams.get('state');
    ...
    // Decode and verify state parameter (CSRF protection)
    if (!encodedCallbackState) {
      throw new Error('State parameter missing from callback');
    }

    let decodedState: { securityState: string; [key: string]: unknown };
    try {
    ...
    if (decodedState.securityState !== storedSecurityState) {
      throw new Error('State parameter mismatch - possible CSRF attack');
    }

    // Extract custom state data (excluding the security state)
    const { securityState, ...customStateData } = decodedState;
...
function clearSessionData(): void {
  sessionStorage.removeItem('oidc_security_state');
