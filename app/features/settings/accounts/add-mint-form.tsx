...
import { cashuAuthService } from '~/features/shared/cashu-auth';
...
export function AddMintForm() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      const authResult = await cashuAuthService.checkAuthRequired(data.mintUrl);
      ...
      if (authResult.requiresClearAuth) {
        ...
        // Store return URL temporarily in sessionStorage (just the URL, not form data)
        sessionStorage.setItem('oidc_return_to', returnToUrl);

        // Use clean redirect URI without query parameters
        const redirectUri = `${origin}/oidc-callback`;

        await cashuAuthService.startAuth(data.mintUrl, redirectUri);
        return;
      }
