...
import { ClosePageButton } from '~/components/page';
...
export default function ReceiveToken({
  ...
  // NOTE: cashu pubkeys are 32-bytes. Need to see whether OS uses compressed pubkeys or x-only
  const canClaim =
  ...
  const isMintTrusted: boolean | undefined = undefined;
        ...
        {isSpentLoading ? (
          ...
          <>
            <div className="flex flex-col items-center">
              <MoneyDisplay money={tokenMoney} simpleFormat />
              {shouldShowConvertedAmount && (
                <MoneyDisplay
                  money={tokenMoney.convert(
                    defaultFiatCurrency,
                    rates.getRate(tokenMoney.currency, defaultFiatCurrency),
                  )}
                  simpleFormat
                  size="sm"
            ...
            {isSpent && (
              <div>
                <p className="text-red-500">Token is spent</p>
              </div>
            )}
