...
export default function ReceiveCashu({ amount, account }: Props) {
  ...
  const { data: checkedQuote, error: checkQuoteError } = useQuery({
    queryKey: ['receive-cashu-status', mintQuote?.quote],
    queryFn: async () => {
      if (!mintQuoteRef.current?.quote) return null;
      const checked = await wallet.checkMintQuote(mintQuoteRef.current.quote);
      mintQuoteRef.current.state = checked.state;
      return checked;
    },
    enabled:
  ...
  console.log('checkedQuote', checkedQuote);
  console.log('mintQuoteError', mintQuoteError);
  console.log('checkQuoteError', checkQuoteError);
  if (loadingMintQuote) {
  ...
  return (
    <>
      ...
      <PageContent>
        ...
        {mintQuoteRef.current?.state === MintQuoteState.PAID && (
