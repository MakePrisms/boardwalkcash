        ...
        async (
          ...
          if (payload.eventType === 'DELETE') {
            ...
            queryClient.resetQueries({
              queryKey: ['cashu-receive-quote', deletedQuote?.id],
              exact: true,
            });
...
type UseCashuReceiveQuoteResponse = {
  status: 'loading' | 'unpaid' | 'expired' | 'paid' | 'completed';
  quote: CashuReceiveQuote | undefined;
};
...
export function useCashuReceiveQuote({
  ...
  if (enabled && !quote && !data) {
    throw new Error('Quote not found');
  }

  let status: UseCashuReceiveQuoteResponse['status'] | undefined;
  ...
  if (!enabled || !quote) {
    ...
    status = 'expired';
  } else if (!data && quote?.state === 'PAID') {
    status = 'completed';
  } else if (quote?.state === 'UNPAID') {
    status = 'unpaid';
  } else if (quote?.state === 'PAID') {
    status = 'paid';
  }
  ...
  if (!status) {
    throw new Error('Unknown quote state');
  }
