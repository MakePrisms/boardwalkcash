...
import { cashuAuthService } from './cashu-auth';
...
export function getCashuWalletWithAuth(
  ...
  return getCashuWallet(mintUrl, {
    getClearAuthToken: async () => {
      const token = await cashuAuthService.getClearAuthToken(mintUrl);
      if (token === null) {
        throw new Error(
    ...
    getAndConsumeBlindAuthToken: async () => {
      return await cashuAuthService.getAndConsumeBlindAuthToken(mintUrl);
    },
    ...options,
  });
}
