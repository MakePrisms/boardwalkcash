...
type State = {
   ...
   */
  destinationPaymentRequest: string | null;
  /**
   ...
   */
  destination: string | Contact | null;
} & (
  | {
...
export const createSendStore = ({
  ...
  return create<SendState>()((set, get) => {
    ...
    return {
      ...
      sendType: 'CASHU_TOKEN',
      destinationPaymentRequest: null,
      destination: null,
      quote: null,
      cashuToken: null,
      ...
      selectDestination: async (destination) => {
        ...
        if (bolt11ParseResult.valid) {
          ...
          set({
            ...
            destinationPaymentRequest: destination,
      ...
      getQuote: async (amount, convertedAmount) => {
        ...
        if (['LN_ADDRESS', 'AGICASH_CONTACT'].includes(sendType)) {
          ...
          const lnAddress = isContact(destination)
            ? destination.lud16
            : destination;
          ...
          try {
            ...
            set({ destinationPaymentRequest: bolt11 });
          } catch (error) {
            console.error(error);
            set({ status: 'idle' });
            return { success: false, error };
        ...
        if (
          ...
          const destinationPaymentRequest = getOrThrow(
            'destinationPaymentRequest',
          );

          try {
            const quote = await createCashuSendQuote({
              account,
              paymentRequest: destinationPaymentRequest,
              amount: amountToSend,
            });
            set({ quote });
          } catch (error) {
