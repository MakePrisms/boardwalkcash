...
function useOnProofStateChange({ swaps, onSpent }: OnProofStateChangeProps) {
  ...
  const spentSwapsRef = useRef<Set<string>>(new Set());
  ...
  const handleProofStateUpdate = useCallback(
    async (proofUpdate: ProofState & { proof: Proof }) => {
      ...
      // TODO: this is a hack to prevent double calls to onSpent
      // we end up calling multiple times if the mint sends up more updates
      if (spentSwapsRef.current.has(swap.id)) return;
      ...
      if (allProofsSpent) {
        ...
        spentSwapsRef.current.add(swap.id);
