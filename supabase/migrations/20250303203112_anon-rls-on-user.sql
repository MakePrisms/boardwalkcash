...
RETURNS TABLE (username text, default_currency text) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  RETURN QUERY
  SELECT u.username, u.default_currency
  FROM wallet.users u 
...
-- TODO: can't add same contact twice. Can probably do this with a unique constraint on owner_id and username.
