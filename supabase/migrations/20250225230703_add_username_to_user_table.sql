...
ALTER TABLE "wallet"."users" ADD CONSTRAINT "users_username_length" 
  CHECK (length(username) >= 3 AND length(username) <= 20);

-- This requirement comes from LUD16 https://github.com/lnurl/luds/blob/luds/16.md
ALTER TABLE "wallet"."users" ADD CONSTRAINT "users_username_format" 
  CHECK (username ~ '^[a-z0-9_-]+$');

CREATE OR REPLACE FUNCTION wallet.is_reserved_username(username TEXT)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN username IN (
    'settings', 'profile', 'login', 'signup', 'api', 'admin', 
    'help', 'about', 'terms', 'privacy', 'receive', 'send', 'verify-email'
  );
END;
$$ LANGUAGE plpgsql IMMUTABLE;

ALTER TABLE "wallet"."users" ADD CONSTRAINT "users_username_not_reserved" 
  CHECK (NOT wallet.is_reserved_username(username));
...
-- Updated: Add username to return value of upsert_user_with_accounts function
