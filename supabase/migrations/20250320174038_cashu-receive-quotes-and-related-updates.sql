...
CREATE OR REPLACE FUNCTION wallet.create_cashu_receive_quote(user_id uuid, account_id uuid, amount numeric, currency text, unit text, quote_id text, payment_request text, expires_at timestamp with time zone, state text, description text DEFAULT NULL::text)
 RETURNS wallet.cashu_receive_quotes
 LANGUAGE plpgsql
AS $function$
declare
