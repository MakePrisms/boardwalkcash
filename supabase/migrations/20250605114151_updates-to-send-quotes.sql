alter table "wallet"."cashu_send_quotes" drop column "amount_to_send";

alter table "wallet"."cashu_send_quotes" drop column "fee_reserve";

alter table "wallet"."cashu_send_quotes" add column "amount_to_receive" numeric not null;

alter table "wallet"."cashu_send_quotes" add column "cashu_fee" numeric not null;

alter table "wallet"."cashu_send_quotes" add column "lightning_fee_reserve" numeric not null;

drop function if exists wallet.create_cashu_send_quote(
    uuid, uuid, text, text, text, timestamp with time zone, numeric, text, bigint, numeric, numeric, text, text, integer, integer, text, integer, text
);

-- Replace function to store cashu fee and rename some parameters
...
drop function if exists wallet.complete_cashu_send_quote(
  uuid, integer, text, numeric, text, integer
);
