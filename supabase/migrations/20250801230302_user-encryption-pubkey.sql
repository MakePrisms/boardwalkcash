...
ALTER TABLE wallet.users ADD COLUMN encryption_public_key text NOT NULL;

DROP FUNCTION IF EXISTS wallet.upsert_user_with_accounts(uuid, text, boolean, jsonb[], text);

-- Update upsert_user_with_accounts function to include encryption_public_key
CREATE OR REPLACE FUNCTION wallet.upsert_user_with_accounts(p_user_id uuid, p_email text, p_email_verified boolean, p_accounts jsonb[], p_cashu_locking_xpub text, p_encryption_public_key text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$declare
  result_user jsonb;
