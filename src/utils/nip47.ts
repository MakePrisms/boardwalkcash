...
export class NIP47Response extends NDKEvent {
   ...
   public buildResponse = async (nwaPrivKey: string) => {
      ...
      const encrypted = await this.encryptContent(nwaPrivKey, this.requestEvent.pubkey, content);
   ...
   private encryptContent = async (nwaPrivKey: string, pubkey: string, content: any) => {
